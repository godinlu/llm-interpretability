...
class TextClassifier(LightningModule):
    ...
    def train_model(self, training_data_loader: DataLoader, validation_data_loader: DataLoader = None, model_name: str = None) -> None:
        self.train()
        logger = CSVLogger(save_dir=self.cfg.logging_root_dir, name=model_name)
        trainer = Trainer(max_epochs=self.cfg.max_epochs,
                          logger=logger)
        trainer.fit(self, training_data_loader, validation_data_loader)
        with open(Path(self.cfg.logging_root_dir) / Path(model_name) / self.cfg.path_to_save_cfg, "wb+") as file:
                dump(self.cfg, file)
